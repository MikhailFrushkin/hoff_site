...
def parse(art):
    try:
        with open('{}.json'.format(art), 'r', encoding='utf-8') as file:
            data = json.load(file)
            return {
                ...
                'slider_picture': data['slider_picture'],
                'price': data['price'],
                'box': data['box']
            }
        ...
        try:
            ...
            if response.get('data').get('characteristic_tab').get('packages') is None:
                box = 1
            else:
                box = len(response.get('data').get('characteristic_tab').get('packages'))
            ...
            if url.startswith('http'):
                ...
                url = 'https://hoff.ru' + url

            data = {
                'id_product': id_product,
                'articul': articul,
                'characteristic': characteristic_list[:3],
                'url': url,
                'name': name,
            ...
            with open("{}.json".format(art), "w", encoding='utf-8') as write_file:
                json.dump(data, write_file, indent=4, ensure_ascii=False)
            return data

        except Exception as ex:
            print('Нет товара на сайте =( {}'.format(ex))
            return {
                'id_product': 0,
                'articul': 0,
                'characteristic': [],
